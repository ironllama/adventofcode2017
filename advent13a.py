...
From here, for the first half of the range (in the previous example, 0 1 2 3) as a modulo to the full range cycle, or depth % range_cycle, since the cycle keeps repeating over and over again. At this point, we know we will hit the scanner if this modulo is 0 -- the top of the scan cycle.

If you want to go one step further, you can get the exact position of the sweep. While not needed for this puzzle, here it is for my own curiosity. The second cell of the 2nd sweep is the same as the second cell of the 15th sweep, is the same as the second cell of the 187th sweep. However, the modulo only works until it his the last half of the range cycle, because the numbers have to come back down, instead of going up. In other words, we would be finished if the numbers were: 0 1 2 3 4 5 6 7. But since the last three are actually 3 2 1, instead of 5 6 7, we have to adjust the math. Each is a shrinking amount from the end of the full cycle range, like -3 -2 -1, so we can describe it as range_cycle - (depth % range_cycle).

For this part, the depth and time are the same, since the packet only goes one depth per picosecond. At 0 picoseconds, it's at the 0 depth. At 1 picoseconds, it's at depth 1. At 5 picoseconds it's at depth 5. And at 98 picoseconds, it's at depth 98. Depth and time are the same! '''
